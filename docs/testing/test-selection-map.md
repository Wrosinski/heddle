...
| Integrated witness authoring, checkpoint, scaffold and final assessment | [scaffold prompt delivery](../../tests/gate/test_document_altitude.py), [admission and assessment freshness](../../tests/runtime/test_document_altitude.py), [kickoff corpus](../../tests/runtime/test_briefing_corpus.py) | Synthetic plans carry the witness design and exact ruling reference; check delivered content and frozen/fresh input identity. Pin delivered instructions for checkpoint recording after specify and assessment ownership at both final boundaries; these do not prove agent compliance. A controlled-review scenario verifies that an Assessment edit invalidates review-backed closure and cannot bypass required originating inspection. Inspect scope and prompt-identity consumers. Installed delivery uses the exact packaged-floor toolchain node below; no full lifecycle or provider permission is implied. Semantic coherence and judgment need explicit review evidence, not token assertions. |
