...
| State/model decisions and completion | [current policy/state](../../tests/kernel/test_tiering_policy.py), [completion boundary](../../tests/runtime/test_tiering_completion.py), [completion repairs](../../tests/runtime/test_completion_repairs.py) | Select retained-report roles and ordering, terminal read parity, pending/conflict routing, cleanup-candidate versus archive-revalidation failure, legacy omission, and workspace-aware review-record repair as applicable. The [installed repair journey](../../tests/runtime/test_completion_repairs_installed.py) and other installed journeys are opt-in e2e. |
...
| Proof continuity | [declarations](../../tests/runtime/test_proof_declarations.py), [early coverage](../../tests/runtime/test_coverage_observation.py), [ownership append](../../tests/runtime/test_owns_append.py), [verification rounds](../../tests/runtime/test_verification_rounds.py), [reaffirmation](../../tests/runtime/test_disposition_reaffirm.py), [source advice](../../tests/gate/test_review_source_advisory.py) | Fast synthetic component boundaries for exact proof declarations, coverage observation, ownership append, target-complete verification rounds, evidence-preserving reaffirmation and selected-source advice. Close-suite reporting cases live in `test_workflow_completion_visibility.py`; help/guidance cases in `test_briefing_corpus.py`. Preserve byte freshness, originating coverage/inspection, stopped/capped/sealed review and attribution-integrity survivors. The [single installed journey](../../tests/runtime/test_proof_continuity_installed.py) is acceptance + e2e + toolchain, not fast and not live. It uses a candidate wheel, native workflow and controlled external transport; exact execution authority is separate from this discovery map. |
...
| CLI routing and envelopes | [dispatch](../../tests/runtime/test_dispatch.py), [contracts](../../tests/runtime/test_contracts.py), [schemas](../../tests/runtime/test_schemas.py), [pending-intake CLI rendering](../../tests/runtime/test_action_contracts.py) | Direct application operations, driver callers, cold subprocess imports. The two pending-intake admission scenarios cover text/JSON parity, confirmed and unconfirmed intakes, multiple-intake selection, explicit selection, next actions, diagnostics and read-only behavior; they also check absent-intake and admitted-feature rendering. |
...
| Attribution of merged outside-feature changes | [native attribution](../../tests/runtime/test_source_attribution.py), [command contract](../../tests/runtime/test_contracts.py) | Exact source and evidence binding, atomic append-only mutation, stale rejection, ownership precedence and unchanged verification requirements; use synthetic Git histories. No full lifecycle or provider run is implied. |
...
| Scaffold primary test bindings | [pure resolver](../../tests/kernel/test_primary_test_bindings.py), [capture and admission](../../tests/gate/test_scaffold_primary_bindings.py), [runtime refusal and retained reviews](../../tests/runtime/test_scaffold_binding_admission.py) | Prove native strict selectors, shared captured bytes and full/basis identity, binding-only missing inputs, refusal before provider/attempt changes, and historical interpretation. The AC-checker nodes in [repository hooks](../../tests/guardrails/test_repository_hooks.py) preserve legacy script selectors and selection in the toolchain band. The [installed reject/repair/review journey](../../tests/runtime/test_scaffold_primary_bindings_installed.py) is acceptance + e2e + toolchain and needs its own exact execution grant. Structural linkage does not establish assertion quality or semantic coverage. |
| Integrated witness authoring, checkpoint, scaffold and final assessment | [scaffold prompt delivery](../../tests/gate/test_document_altitude.py), [admission and assessment freshness](../../tests/runtime/test_document_altitude.py), [kickoff corpus](../../tests/runtime/test_briefing_corpus.py) | Synthetic plans carry the witness design and exact ruling reference; check delivered content and frozen/fresh input identity. Review-stage Kickoff guidance uses Orient's configured workspace and distinguishes optional workflow review records from owned product assessments. A controlled-review scenario verifies that an Assessment edit invalidates review-backed closure and cannot bypass required originating inspection. Inspect scope and prompt-identity consumers. Installed delivery uses the exact packaged-floor toolchain node below; no full lifecycle or provider permission is implied. Semantic coherence and judgment need explicit review evidence, not token assertions. |
...
| Installed feature lifecycle | [completion installed](../../tests/runtime/test_completion_installed.py), [identity/readiness installed](../../tests/runtime/test_identity_readiness_installed.py), [completion feedback contract](../../tests/runtime/test_completion_feedback_installed.py) | Explicit local e2e execution. The completion-feedback witness builds and installs the candidate wheel, uses a nondefault workspace, exercises retained-response interpretation without provider retry, legal record repair, full retained-report and terminal parity, archive conflict/recovery, and publishes fixture evidence before disposable-host cleanup. Controlled providers do not remove the e2e requirement. |
